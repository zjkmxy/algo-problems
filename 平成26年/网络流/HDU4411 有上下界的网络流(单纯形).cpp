/*
hdu 4411 Arrest
最小查找62ms，离开查找31ms（最好成绩）
开LCT估计能到0ms，不想写了（代码量爆表啊……）
*/
#include<cstdio>
#include<cstring>
#include<iostream>
...
int calc(int s)
{
	...
	for(i=1;i<=n;i++)
	{
		addedge(i*2, i*2+1, 1, 1, 0, ecnt);
		addedge(sp2, i*2, 0, s, dis[0][i], ecnt);
		addedge(i*2+1, tp, 0, s, dis[0][i], ecnt);
	}
	...
	for(i=1;i<=n;i++)
	{
		for(j=i+1;j<=n;j++)
		{
			addedge(i*2+1, j*2, 0, 1, dis[i][j], ecnt);
		}
	}
	addedge(sp2, tp, 0, s, 0, ecnt);
	if(!NSMCalc(ret))
		return -1;
	return ret;
}
