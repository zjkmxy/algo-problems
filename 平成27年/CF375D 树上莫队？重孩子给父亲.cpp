/*
每个节点有一种颜色，每次问一个节点的子树中所有个数超过某个定值的颜色有多少种。
DFS计算，先递归处理每个非重儿子，清理个数数组。
然后递归重儿子，这时候不清理数组，而是把所有非重儿子子树的颜色都插入到重儿子中去。
离线可以直接出答案了，在线的话我们需要维护一个可持久化线段树。
因为每个节点至多只会被插入O(logN)次（就是它到1号节点的路径长度），所以复杂度是O(NlogNlogN)
关键的是，空间复杂度也是O(NlogNlogN)的，所以需要强优化。这里利用了颜色总个数的性质。
颜色种类数越多，线段树上修改操作越频繁，但是线段树本身不用开那么大。
反之颜色种类数越少，线段树越大，但是修改操作不那么频繁了。
*/
#include <cstdio>
#include <cstring>
#include <vector>
